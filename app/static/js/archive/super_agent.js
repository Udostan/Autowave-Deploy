...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function startTaskPolling(taskId) {
        ...
        const taskDesc = taskDescription ? taskDescription.value.trim() : "this task";

        // We're using the autowave template as our primary template
        console.log('Using template: autowave');

        // Start simulated thinking immediately using the appropriate handler
        if (window.startThinkingProcess) {
            window.startThinkingProcess(taskDesc);
            console.log('Started thinking process using handler');
        } else {
            console.error('Thinking process handler not loaded');

            // Fallback to basic thinking display if no handler is available
            if (thinkingContent) {
                thinkingContent.innerHTML = '<p>Analyzing your request and preparing execution plan...</p>';
                console.log('Using fallback thinking display');
            }
        }

        // Start polling only for task completion status
        taskPollingInterval = setInterval(() => {
            // Make API request to get task status
            fetch(`/api/prime-agent/task-status?task_id=${taskId}`)
                .then(response => response.json())
                .then(data => {
                    console.log('Task status:', data);

                    if (data.success) {
                        // If task is complete, stop polling and simulated thinking
                        if (data.status === 'complete') {
                            clearInterval(taskPollingInterval);

                            // Stop simulated thinking using our handler
                            if (window.stopThinkingProcess) {
                                window.stopThinkingProcess();
                                console.log('Stopped thinking process using handler');
                            }

                            console.log('Task completed, stopping polling and thinking simulation');
                        }
                    }
                })
                .catch(error => {
    ...
    // We've moved all thinking process functionality to thinking_process.js
    ...
    if (executeTaskBtn) {
        ...
        executeTaskBtn.addEventListener('click', function() {
            ...
            // Clear the thinking content and make sure it's visible
            if (thinkingContent) {
                thinkingContent.innerHTML = '<p style="color: #000000; font-weight: bold;">Starting analysis for: ' + description + '</p>';
                thinkingContent.style.display = 'block';
                thinkingContent.style.visibility = 'visible';
                console.log('Cleared thinking content and added initial message');
            } else {
            ...
            if (thinkingContainer) {
                ...
                console.log('Made thinking container visible');
            }
            ...
            if (thinkingProcess) {
                ...
                thinkingProcess.style.backgroundColor = '#f0f0f0';
                console.log('Made thinking process visible');
            }
            ...
            const requestData = {
                task: description,
                use_visual_browser: useAdvancedBrowser ? useAdvancedBrowser.checked : true  // Always use advanced browser
            };
            ...
            .then(data => {
                ...
                // If we have a task_id, start polling for real-time updates
                if (data.task_id) {
                    console.log('Starting task polling with task_id:', data.task_id);
                    startTaskPolling(data.task_id);
                } else if (data.success && data.message && data.message.includes('Task executed successfully')) {
                    // Extract task_id from the response if available
                    const taskIdMatch = JSON.stringify(data).match(/"task_id":"([^"]+)"/);
                    if (taskIdMatch && taskIdMatch[1]) {
                        const extractedTaskId = taskIdMatch[1];
                        console.log('Extracted task_id from response:', extractedTaskId);
                        startTaskPolling(extractedTaskId);
                    }
                }
